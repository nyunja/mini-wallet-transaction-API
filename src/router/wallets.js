 ...
 */
router.post("/:id/deposit", (req, res, next) => {
  try {
    const walletId = Number(req.params.id);
    ...
    if (!amount || typeof amount !== "number" || amount <= 0) {
      return res.status(400).json({ error: "Amount should be a valid number greater than 0" });
    }
    ...
    const tx = db.transaction(() => {
      ...
      db.prepare("INSERT INTO transactions (type, to_wallet_id, from_wallet_id, amount) VALUES (?, ?, ?, ?)").run("deposit", walletId, null, amount);
    });
...
router.post("/:id/transfer", (req, res, next) => {
  try {
    ...
    if (!fromWalletId) return res.status(400).json({ error: "Invalid sender wallet id" });

    if (!toWalletId) return res.status(400).json({ error: "Invalid receiver wallet id" });
